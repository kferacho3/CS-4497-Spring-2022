public class wpg
{
  ...
  public void setLatticeVectors()
  {
    ...
    {
      this.V = Va(nV, U(R(this.U, 2*PI/3, V(1,0,0), V(0,-1,0))));
    }
    
    //60-degrees
    if (curr_lattice.contains("H"))
    {
      this.V = Va(nV, U(R(this.U, PI/3, V(1,0,0), V(0,-1,0))));
    }
    
    //Rectangular
    if (curr_lattice.contains("N"))
    {
      this.V = Va(nV, M(U(R(this.U))));
    }
    
    //Rhombic
    if (curr_lattice.contains("E"))
    {
      this.U = Va(sqrt(nU*nV), U(this.U));
      this.V = Va(sqrt(nU*nV), U(this.V));
    }
  }
  ...
  pts getMasterTile()
  {
    ...
    if (wallpaper_id == 1)
    {
      output.addPt(P(this.O, this.U));
      output.addPt(P(this.O));
      output.addPt(P(this.O, Va(0.5, this.V)));
      output.addPt(P(this.O, A(this.U, Va(0.5, this.V))));
    }
    if (wallpaper_id == 2)
    {
      output.addPt(P(this.O, this.U));
    ...
    if (wallpaper_id == 5)
    {
      output.addPt(P(this.O, Va(0.5, this.U)));
      output.addPt(P(this.O));
      output.addPt(P(this.O, Va(0.5, this.V)));
