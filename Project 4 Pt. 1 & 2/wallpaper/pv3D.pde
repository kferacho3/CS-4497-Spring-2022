...
class vec { float x=0,y=0,z=0; 
   ...
   vec rotate(float a, vec I, vec J) { // Rotate this by angle a parallel in plane (I,J) Assumes I and J are orthogonal
     ...
     return this; };
   void print() {println(this.x + " " + this.y + " " + this.z);}
   } // end class vec
...
vec V(float a, vec A, float b, vec B) {return A(Va(a,A),Va(b,B));}                                       // aA+bB 
vec V(float a, vec A, float b, vec B, float c, vec C) {return A(V(a,A,b,B),Va(c,C));}                   // aA+bB+cC
vec V(pt P, pt Q) {return new vec(Q.x-P.x,Q.y-P.y,Q.z-P.z);};                                          // PQ
vec U(vec V) {float n = V.norm(); if (n<0.0000001) return V(0,0,0); else return Va(1./n,V);};           // V/||V||
...
vec slerp(vec V1, vec V2, float t) {
  float a = angle(V1,V2);
  return A(Va(sin((1-t)*a)/sin(a),V1), Va(sin(t*a)/sin(a), V2));
}
...
float angle(vec U, vec V) {if (abs((d(U,V)/n(V)/n(U) - 1)) < 0.0001) return 0;
  else if (abs((d(U,V)/n(V)/n(U) + 1)) < 0.0001) return PI;
  return acos(d(U,V)/n(V)/n(U)); };// angle(U,V)
float angleSigned(vec U, vec V) {
  if (abs((d(U,V)/n(V)/n(U) - 1)) < 0.0001) return 0;
  else if (abs((d(U,V)/n(V)/n(U) + 1)) < 0.0001) return PI;
  float s = det2(U,V) > 0 ? 1 : -1;
  return s*acos(d(U,V)/n(V)/n(U));
}
...
boolean isEqual(pt P1, pt P2) {return (P1.x==P2.x)&&(P1.y==P2.y)&&(P1.z==P2.z); }
...
void show(pt P, float r, color c) {noStroke(); fill(c); pushMatrix(); translate(P.x,P.y,P.z); sphere(r); popMatrix();}; // render sphere of radius r and center P
...
void arrow(pt P, vec V, float r) {
  stub(P,Va(.8,V),r*2/3,r*2/3); 
  cone(P(P,Va(.8,V)),Va(.2,V),1.5*r); 
  }  
...
vec ToK(vec V) {
 ...
 //*********** Concave
 // true if the edge 
boolean convex(pt A, pt B, pt C, pt O, boolean abISshort, boolean bcISshort) {
   return !(m(O,A,B,C)>0 != abISshort != bcISshort) ;
   }
   


float clockwise(vec U, vec V, vec W) {
    float result=0;
    result=det2(M(U(V),U(U)),M(U(W),U(V)));
    return result;
  }
