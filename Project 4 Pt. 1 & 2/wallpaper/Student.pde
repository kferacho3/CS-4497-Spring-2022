  ...
  switch(mode) {
    ...
    pushMatrix();
        ...
        for (int i=0; i<=circleResolution; i++) {
          float xShape = 0 + cos(angle*i) * radsss;
          float yShape = 0 + sin(angle*i) * radsss;
          ...
          switch (threeDm) {
           ...
           case 0:
             ...
             if (i % 512 ==0) {
             if (tex) {
              for (int j=0; j<=int(circleResolution /500); j++) {
                  float sh = 0 + cos(angle*i) * radsss;
                  float shh = 0 + sin(angle*i) * radsss;
         
                  //vertex(xShape, yShape);
                translate(20 * t,40 * t);
               Planets(sh  / 10, shh, rand1, planets[plans], textures[text]);
              }
             } 
             if (!tex) {
               
               
               Planets(xShape  / 10, yShape, rand1, planets[plans], textures[text]);
             }
             break;
...
void Planets(float r, float d, float o, PImage img, PImage img2) {
    ...
    texturee = createShape(SPHERE, radsss);
