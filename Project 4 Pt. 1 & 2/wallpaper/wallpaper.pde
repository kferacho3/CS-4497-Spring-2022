...
PImage[] textures = new PImage[29];
PImage[] planets = new PImage[100];

int text = int(random(29));
int plans = int(random(100));
void setup() {
textureMode(NORMAL);
    for (int k = 0; k < textures.length; k++) {
         String num = str(k);
         textures[k] = loadImage("miniplanet/t" + num + ".jpeg");
         
       }
    
       
                //load textures for main planet
   for (int l = 0; l < planets.length; l++) {
       String num = str(l);
                  try{
                    planets[l] = loadImage("portraits3/portrait" + num + ".jpg");
  ...
  WP.wallpaper_id = 1;
  WP.setSimilarityVectors(Va(a, U_direction), Va(a, V_direction));
...
void draw() {
  ...
  if (scribeText)
  {
    ...
    for (int i = 0; i < WP.num_s; i++)
    {
      ...
      if (((WP.wallpaper_id == 2) || (WP.wallpaper_id == 5) || (WP.wallpaper_id ==  13)) &&  (WP.S[i].type =="MIRROR")) {
        extra = "AXIS";
      }
...
String title ="Warped wallpaper patterns", name ="Kamal Feracho",
